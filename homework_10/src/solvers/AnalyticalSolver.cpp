...
// додатний корінь. Кидає Homework10Error, якщо рівняння не має додатного
...
double solve_cardano_time(double cubic_a, double cubic_b, double cubic_c)
{
  if (std::fabs(cubic_a) < kCubicCoeffEpsilon) {
    throw Homework10Error{"flight time equation degenerates (a ~ 0)"};
  }
  ...
  if (depressed_p >= 0.0) {
    throw Homework10Error{"flight time equation has no real positive root (p >= 0)"};
  }
  ...
  if (acos_arg < kAcosArgLowerBound || acos_arg > kAcosArgUpperBound) {
    throw Homework10Error{"flight time equation has complex roots (|acos arg| > 1)"};
  }
  ...
  if (smallest_positive < 0.0) {
    throw Homework10Error{"flight time equation has no positive root"};
  }

...
{
  if (axis.size() < 2) {
    throw Homework10Error{"ballistic table axis " + name + " must contain at least two values"};
  }
  if (!std::is_sorted(axis.begin(), axis.end())) {
    throw Homework10Error{"ballistic table axis " + name + " must be sorted"};
  }
  for (std::size_t i = 1; i < axis.size(); ++i) {
    if (std::fabs(axis[i] - axis[i - 1]) <= kEpsilon) {
      throw Homework10Error{"ballistic table axis " + name + " contains duplicate values"};
    }
  }
}
...
std::vector<double> read_axis(std::ifstream& input, std::size_t count, const std::string& name)
{
  ...
  for (double& value : axis) {
    if (!(input >> value)) {
      throw Homework10Error{"cannot read ballistic table axis " + name};
    }
  }
  ...
  std::size_t iz, std::size_t iv, std::size_t im, std::size_t id, std::size_t il) const
{
  ...
  if (flat >= data.size()) {
    throw Homework10Error{"ballistic table index out of range"};
  }
  return data[flat];
}
...
void BallisticTable::load(const std::string& path)
{
  ...
  if (!input) {
    throw Homework10Error{"cannot open ballistic table: " + path};
  }
  ...
  if (!(input >> nZ >> nV >> nM >> nD >> nL)) {
    throw Homework10Error{"cannot read ballistic table dimensions: " + path};
  }
  ...
  for (BallisticTableResult& result : data) {
    if (!(input >> result.time_of_flight >> result.horizontal_distance)) {
      throw Homework10Error{"not enough ballistic table result rows: " + path};
    }
    if (result.time_of_flight <= 0.0 || result.horizontal_distance <= 0.0) {
      throw Homework10Error{"ballistic table results must be positive"};
    }
  }
}
...
BallisticTableResult BallisticTable::lookup(double z0, double v0, double mass, double drag, double lift) const
{
  ...
  if (data.size() != expected) {
    throw Homework10Error{"ballistic table data size does not match axes"};
  }
...
{
  if (altitude <= 0.0) {
    throw Homework10Error{"altitude must be positive"};
  }
  if (attack_speed < 0.0) {
    throw Homework10Error{"attack speed must be non-negative"};
  }
  if (ammo.mass <= 0.0) {
    throw Homework10Error{"ammo mass must be positive"};
  }
  ...
  if (!std::isfinite(horizontal_distance) || horizontal_distance <= kEpsilon) {
    throw Homework10Error{"horizontal distance must be positive and finite"};
  }
  ...
  if (distance <= kEpsilon) {
    throw Homework10Error{"distance to target must be positive"};
  }

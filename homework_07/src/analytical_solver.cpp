...
// додатний корінь. Кидає Homework07Error, якщо рівняння не має додатного
...
double solve_cardano_time(double cubic_a, double cubic_b, double cubic_c)
{
  if (std::fabs(cubic_a) < kCubicCoeffEpsilon) {
    throw Homework07Error{"flight time equation degenerates (a ~ 0)"};
  }
  ...
  if (depressed_p >= 0.0) {
    throw Homework07Error{"flight time equation has no real positive root (p >= 0)"};
  }
  ...
  if (acos_arg < kAcosArgLowerBound || acos_arg > kAcosArgUpperBound) {
    throw Homework07Error{"flight time equation has complex roots (|acos arg| > 1)"};
  }
  ...
  throw Homework07Error{"flight time equation has no positive root"};
}

// Горизонтальна дальність боєприпасу за час t. П'ять членів полінома —
// це та сама форма, що використовувалася в ДЗ1: тут вона потрібна, щоб
// відтворити числові значення з референсного матеріалу курсу.
...
// Сигнатура solve() диктується інтерфейсом і лекцією 14: цілих п'ять
// параметрів. Глушимо bugprone-easily-swappable-parameters локально —
// перейменування або групування зробило б реалізацію менш зрозумілою.
...
{
  if (altitude <= 0.0) {
    throw Homework07Error{"altitude must be positive"};
  }
  if (attack_speed < 0.0) {
    throw Homework07Error{"attack speed must be non-negative"};
  }
  if (ammo.mass <= 0.0) {
    throw Homework07Error{"ammo mass must be positive"};
  }
  ...
  if (!std::isfinite(horizontal_distance) || horizontal_distance <= kEpsilon) {
    throw Homework07Error{"horizontal distance must be positive and finite"};
  }
  ...
  if (distance <= kEpsilon) {
    throw Homework07Error{"distance to target must be positive"};
  }

  // Точка скиду = ціль мінус вектор від дрона до цілі завдовжки
  // horizontal_distance (тобто дрон підлітає рівно на цю дистанцію до цілі).

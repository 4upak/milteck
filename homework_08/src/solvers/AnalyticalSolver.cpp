...
// додатний корінь. Кидає Homework08Error, якщо рівняння не має додатного
...
double solve_cardano_time(double cubic_a, double cubic_b, double cubic_c)
{
  if (std::fabs(cubic_a) < kCubicCoeffEpsilon) {
    throw Homework08Error{"flight time equation degenerates (a ~ 0)"};
  }
  ...
  if (depressed_p >= 0.0) {
    throw Homework08Error{"flight time equation has no real positive root (p >= 0)"};
  }
  ...
  if (acos_arg < kAcosArgLowerBound || acos_arg > kAcosArgUpperBound) {
    throw Homework08Error{"flight time equation has complex roots (|acos arg| > 1)"};
  }
  ...
  if (smallest_positive < 0.0) {
    throw Homework08Error{"flight time equation has no positive root"};
  }
...
{
  if (altitude <= 0.0) {
    throw Homework08Error{"altitude must be positive"};
  }
  if (attack_speed < 0.0) {
    throw Homework08Error{"attack speed must be non-negative"};
  }
  if (ammo.mass <= 0.0) {
    throw Homework08Error{"ammo mass must be positive"};
  }
  ...
  if (!std::isfinite(horizontal_distance) || horizontal_distance <= kEpsilon) {
    throw Homework08Error{"horizontal distance must be positive and finite"};
  }
  ...
  if (distance <= kEpsilon) {
    throw Homework08Error{"distance to target must be positive"};
  }

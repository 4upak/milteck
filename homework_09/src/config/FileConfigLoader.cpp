...
nlohmann::json read_json_file(const std::filesystem::path& path)
{
  ...
  if (!input) {
    throw Homework09Error{"cannot open " + path.string()};
  }
  nlohmann::json data;
  try {
    input >> data;
    ...
    throw Homework09Error{"invalid JSON in " + path.string() + ": " + parse_error.what()};
  }
  return data;
}
...
std::unordered_map<std::string, AmmoParams> load_ammo_table(const nlohmann::json& data, const std::filesystem::path& path)
{
  if (!data.is_array() || data.empty()) {
    throw Homework09Error{path.string() + " must be a non-empty array"};
  }
...
void FileConfigLoader::load(const std::string& source)
{
  ...
  if (it == ammo_table_.end()) {
    throw Homework09Error{"unknown ammo type: " + config_.ammo_name};
  }
...
{
  if (!loaded_) {
    throw Homework09Error{"FileConfigLoader: getConfig() called before load()"};
  }
  return config_;
}
...
{
  if (!loaded_) {
    throw Homework09Error{"FileConfigLoader: getAmmoParams() called before load()"};
  }
...
{
  if (!loaded_) {
    throw Homework09Error{"FileConfigLoader: ammoTable() called before load()"};
  }
  return ammo_table_;
}

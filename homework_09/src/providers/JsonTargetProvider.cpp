...
Coord read_coord(const nlohmann::json& node, const std::string& field)
{
  if (!node.contains(field) || !node.at(field).is_object()) {
    throw Homework09Error{"target field '" + field + "' must be an object"};
  }
  const nlohmann::json& obj = node.at(field);
  if (!obj.contains("x") || !obj.contains("y") || !obj.at("x").is_number() || !obj.at("y").is_number()) {
    throw Homework09Error{"coordinate '" + field + "' must contain numeric x and y"};
  }
...
JsonTargetProvider::JsonTargetProvider(const std::string& path)
{
  ...
  if (!input) {
    throw Homework09Error{"cannot open targets file: " + path};
  }
  ...
  try {
    ...
    throw Homework09Error{"invalid JSON in " + path + ": " + parse_error.what()};
  }

  if (!data.contains("targets") || !data.at("targets").is_array()) {
    throw Homework09Error{"targets file must contain a 'targets' array"};
  }
  const nlohmann::json& items = data.at("targets");
  if (items.empty()) {
    throw Homework09Error{"targets array is empty in " + path};
  }
  ...
  for (const nlohmann::json& entry : items) {
    if (!entry.is_object()) {
      throw Homework09Error{"target #" + std::to_string(json_index) + " must be an object"};
    }
